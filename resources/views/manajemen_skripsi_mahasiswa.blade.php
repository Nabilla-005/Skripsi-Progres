@extends('mylayout.mainlayout')

@section('content')
    <div class="container mt-5">
        <h2 class="mb-4 text-center" style="font-weight: 600; color: #3a3a3a;">Manajemen Skripsi Mahasiswa</h2>
        
        <!-- Menampilkan daftar mahasiswa dan status pengajuan skripsi -->
        <div class="row">
            <div class="col-12">
                <div class="table-responsive">
                    <table class="table table-hover table-striped shadow-sm">
                        <thead class="thead-dark" style="background-color: #343a40; color: #fff;">
                            <tr>
                                <th>Nama Mahasiswa</th>
                                <th>Status Pengajuan</th>
                                <th>Status Progres Skripsi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($mahasiswas as $mahasiswa)
                                <tr style="background-color: #f8f9fa;">
                                    <td>{{ $mahasiswa->nama }}</td>
                                    <td>
                                        @if ($mahasiswa->pengajuanJuduls->isNotEmpty())
                                            <span class="badge bg-success">{{ $mahasiswa->pengajuanJuduls->last()->status }}</span>
                                        @else
                                            <span class="badge bg-warning">Belum ada pengajuan</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($mahasiswa->progresSkripsis->isNotEmpty())
                                            <span class="badge bg-success">{{ $mahasiswa->progresSkripsis->last()->komentar }}</span>
                                        @else
                                            <span class="badge bg-warning">Belum ada progres</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
@endsection
